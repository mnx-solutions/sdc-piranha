...
function setupManta(client, subuserObjName, path, callback) {
    var waitingTime = Date.now() + 5 * 60 * 1000;
    var trySettingRoleTag = function () {
        client.setRoleTags(path, [subuserObjName], true, function () {
            client.getRoleTags(path, function (getErr, roles) {
                if (roles && roles.indexOf(subuserObjName) !== -1) {
                    callback(null);
                } else if (Date.now() > waitingTime) {
                    callback('Cannot set role tags for ' + subuserObjName + ' folder');
                } else {
                    setTimeout(trySettingRoleTag, 1000);
                }
            });
        });
    };
    ...
    call.cloud.listUsers(function (listErr, users) {
        ...
        function setupSubuserAndManta() {
            setupSubuser(call, keyPair, subuserLogin, subuserObjName, options.listRules, function (err) {
                ...
                setupManta(client, subuserObjName, path, callback);
            });
        }
