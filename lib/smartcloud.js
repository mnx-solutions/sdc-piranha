...
SmartCloud.prototype.cloud = function (opts, noCache, callback) {
    ...
    if(_tokenCloud[opts.token] && !callback && !noCache) {
        if(++_tokenCount[opts.token] < self._maxSimultaneous) {
    ...
    Object.defineProperty(cloud, '_clients', {
        value: {}
    });
    var client = smartdc.createClient(options);
    cloud._clients[self._url] = client;
        ...
        value: function (url) {
            ...
            if(url && url !== this._currentDC) {
                if(!cloud._clients[url]) {
                    ...
                    Object.defineProperty(options, 'agent', {value: getAgent(options)});
                    cloud._clients[url] = smartdc.createClient(options);
                }
                this._currentDC = url;
            }
        }
    ...
    Object.keys(cloud._client).forEach(function (k) {
        cloud[k] = function(a1, a2, a3, a4, a5, a6, a7, a8) {
            if(!noCache) {
                refreshTimeout(cloud, self._cloudTimeout, self._simultaneousTimeframe);
            }
            return cloud._clients[cloud._currentDC][k](a1, a2, a3, a4, a5, a6, a7, a8);
        };
    });
