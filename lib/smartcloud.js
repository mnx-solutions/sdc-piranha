...
SmartCloud.prototype.cloud = function (opts, noCache, callback) {
    ...
    cloud.listDatacenters = function (callback) {
        if(callback) {
            setImmediate(callback.bind(callback, null, self.datacenters));
        }
        return self.datacenters;
    };

    function handleDatacenterList(err, dcs) {
        if (err) {
            cloud.log.warn('Failed to list datacenters from %s', self._urls[self._main], err);
            if(!self._urls[self._main + 1]) {
                callback(err);
                return;
            }
            self._main++;
            ...
            cloud._client.listDatacenters(handleDatacenterList);
            return;
        }
        if(dcs[undefined]) {
