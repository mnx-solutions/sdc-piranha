...
FileResolver.prototype.resolveFile = function (file, callback) {
	...
	if (file.indexOf('//') === 0 || file.indexOf('https://') === 0 || file.indexOf('http://') === 0) {
		callback(null, {url: file, file: false});
		return;
	}
...
FileResolver.prototype.searchFile = function (component, filePath, callback) {
	...
	fs.exists(absFilePath, function (exists) {
		if (exists) {
			callback('extensions', absFilePath);
			return;
		}
		absFilePath = self.resolveStaticFile(component, filePath, 'modules');
		fs.exists(absFilePath, function (exists2) {
			if (exists2) {
				callback('modules', absFilePath);
				return;
			}
			callback(false);
		});
