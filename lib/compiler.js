...
module.exports = function (rack, config) {
	...
	return function compiler (locals, callback) {
        ...
        if (locals.lang) {
            locals.lang.forEach(function (lang) {
                var rootPath = lang._extension ? '/extensions' : '/site';
                if (lang._scope && lang._scope.type === 'module') {
                    rootPath += '/modules' + lang._url;
                    Localization.load(lang._scope.id, process.cwd() + rootPath);
                }
            });
        }
